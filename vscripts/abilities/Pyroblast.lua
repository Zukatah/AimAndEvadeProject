...
function Pyroblast_MissileUpdate(timerindex)
	...
	local timerIndexSound2 = AAE.timerTable[timerindex].timerIndexSound2
	...
	StopSoundOnUnit(timerIndexSound2)
...
function Pyroblast_ChannelUpdate (timerIndex)
	...
	local timerIndexSound = AAE.timerTable[timerIndex].timerIndexSound
	AAE.timerTable[timerIndex].intervalCount = intervalCount
	...
	local timerIndexSound2 = PlaySoundOnUnitInit("Hero_Phoenix.SuperNova.Cast", missileDummy, 2.0, true)

	AAE.timerTable[timerIndex] = { caster = caster, intervalCount = 0, normVecDir = normVecDir, missileDummy = missileDummy, missileLoc = castLoc, channelStr = intervalCount, cliffLevel = cliffLevel, timerIndexSound2 = timerIndexSound2 }
...
function OnSpellStart ( keys )
	...
	local timerIndexSound = PlaySoundOnUnitInit("Hero_Leshrac.Diabolic_Edict_lp", castDummy, 2.0, true)
	
	local timerIndex = GetTimerIndex()
	AAE.timerTable[timerIndex] = { caster = caster, intervalCount = 0, normVecDir = normVecDir, castDummy = castDummy, castLoc = casterLoc, cliffLevel = cliffLevel, channelStartTime = GameRules:GetGameTime(), timerIndexSound = timerIndexSound }
